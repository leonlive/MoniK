...
APP_TITLE = "MoniK V6 NO STATUS MATCH EXACT TOKEN IDENTITY"
...
def nmap_6668_scan() -> dict:
    """
    Execute exactly:

        nmap -p 6668 -Pn --open --traceroute 192.168.178.0/24
    ...
    try:
        ...
        return {
            "success": False,
            "error": "nmap_timeout_180_seconds",
            "command": " ".join(command),
            "raw_output": output,
            "candidates": [],
            "candidate_count": 0,
    ...
    return {
        "success": run.returncode == 0 or bool(unique),
...
def _apply_lan_matches(model, nmap_rows, matches, source):
    ...
    model["lan_scan"] = {
        **model.get("lan_scan", {}),
        ...
        "command": (
            "nmap -p 6668 -Pn --open --traceroute "
            "192.168.178.0/24"
        ),
        ...
        "status_used_for_matching": False,
        "commands_sent": False,
        "installs": False,
    }
...
def apply_arp(model):
    """
    Exact LAN identity matching without status().
    ...
    print(
        ...
        "UDP identity rows="
        f"{len(udp_rows)}; status comparison=NO.",
        flush=True,
    )
    return model
